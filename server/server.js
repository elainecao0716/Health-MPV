...
const SAFETY_INSTRUCTIONS =
  ...
  "since many factors can be involved. Clearly state uncertainty when you're not sure. If the user describes " +
  "urgent or severe symptoms, tell them to contact emergency services or a qualified medical professional " +
  "right away. Keep answers supportive, concise, and based only on the records provided. If a saved goal " +
  "weight is provided, you may discuss progress toward it using the given data, but never promise a timeline " +
  "or specific date for reaching it, and never recommend rapid or unsafe weight-loss methods.";
...
app.post("/api/coach", async (req, res) => {
  const { records, goalWeight, checkins } = req.body ?? {};

  if (!Array.isArray(records)) {
    return res.status(400).json({ error: "'records' must be an array." });
  }

  // Optional field — old clients that don't send it still work, just without check-in context.
  ...
  try {
    ...
    console.log(
      `[server] POST /api/coach — ${summary.length} record(s), ${checkinSummary.length} check-in(s), goal: ${goalWeight ?? "none"}`
    );
      ...
      messages: [
        {
          ...
          content:
            ...
            "water, exercise, mood, notes), and an optional saved goal weight, identify trends, celebrate progress, gently " +
            "flag any concerning patterns, and suggest 2-4 concrete, actionable next steps. You may discuss general " +
            "patterns involving sleep, hydration, exercise, mood, and weight, but never state guaranteed conclusions " +
            "about cause and effect — describe possible patterns cautiously, since many factors can be involved. If a " +
            "goal weight is provided, you may discuss progress toward it using the given data, but never promise a " +
            "timeline or specific date for reaching it, and never recommend rapid or unsafe weight-loss methods. If the " +
            "user describes urgent or severe symptoms, tell them to contact emergency services or a qualified medical " +
            "professional. Keep the tone warm and non-judgmental. Respond in short paragraphs and/or a brief bullet " +
            "list, under 220 words, and end with a one-line reminder that this is not medical advice.",
        },
        {
          role: "user",
          content:
...
app.post("/api/chat", async (req, res) => {
  const { messages, records, goalWeight, checkins } = req.body ?? {};
  ...
  try {
    ...
    console.log(
      `[server] POST /api/chat — ${history.length} message(s), ${summary.length} record(s), ${checkinSummary.length} check-in(s), goal: ${goalWeight ?? "none"}`
    );
      ...
      messages: [
        {
          ...
          content:
            ...
            "You may discuss general patterns involving sleep, hydration, exercise, mood, and weight using the data " +
            `provided. ${SAFETY_INSTRUCTIONS} ${recordsContext(summary)} ${goalContext(goalWeight)} ${checkinsContext(checkinSummary)}`,
        },
        ...history,
      ],
    });
